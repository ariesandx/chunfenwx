...
import com.chunfen.tomcat.nio.reactor.MainReactor;
...
public class ReactorTest extends BaseTest {
    ...
    @Test
    public void mainSubReactorTest() throws Exception{

        new Thread(new MainReactor(8889)).start();
//        new Thread(new SubReactor()).start();
        ...
        try (SocketChannel socketChannel = SocketChannel.open(new InetSocketAddress(InetAddress.getLocalHost(), 8889))){
            ...
            System.out.println(Thread.currentThread().getName() + " ReactorTest client send success");
            ByteBuffer input = ByteBuffer.allocate(1024);
            socketChannel.read(input);
            ...
            Message obj = objectMapper.readValue(bytes, Message.class);
            System.out.println(Thread.currentThread().getName() + " ReactorTest client read success " + obj.toString());
